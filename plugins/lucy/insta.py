...
async def link_handler(Mbot, message):
    ...
    try:
        ...
        if url.endswith("="):
           dump_file=await message.reply_video(url[:-1],caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TgxFilmBot")
        else:
            dump_file=await message.reply_video(url,caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TgxFilmBot")
        ...
        try:
            if "/reel/" in url:
               ...
               try:
                   if ddinsta:
                      dump_file=await message.reply_video(content_value,caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TgxFilmBot")
                   else:
                       dump_file=await message.reply_video(content_value, caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TgxFilmBot")
               except:
                   ...
                   dump_file=await message.reply_video(downfile,caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TgxFilmBot") 
            elif "/p/" in url:
                  meta_tag = requests.post("https://saveig.app/api/ajaxSearch", data={"q": link, "t": "media", "lang": "en"}, headers=headers)
                  if meta_tag.ok:
                     res=meta_tag.json()
              ...
              #    await message.reply(meta)
                  for i in range(len(meta) - 1):
                     ...
                     try:
                        dump_file=await message.reply_video(com.text,caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TgxFilmBot")
                        await com.delete()
                     except:
                         pass 
            elif "stories" in url:
                  ...
                  if meta_tag.ok:
                     ...
                     meta=re.findall(r'href="(https?://[^"]+)"', res['data']) 
                      ...
                      return await message.reply("Oops something went wrong")
                  try:
                     dump_file=await message.reply_video(meta[0], caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TgxFilmBot")
                  except:
          ...
          #     await message.reply(tracemsg)
            ...
            if 'downfile' in locals():
                os.remove(downfile)
            await message.reply("<a href='https://t.me/TgxFilmBot'>ᴜsᴇ ɴᴇᴡ ғᴇᴀᴛᴜʀᴇs</a>")
